...
let lk = 	[7,14,21,28,32,35,38,42,44,49,50,52,56,60,63,68] 
...
let ls2 = [22,23,24,25,26,28,34,35,,43,44,45,46,47,55,56,62,64,65,66,67,68] //wider s
...
let allLetters = [la,la2,lb,lb2,lc,ld,ld2,le,lf,lf2,lg,lh,lh2,li,li2,lj,lj2,lk,ll,ll2,ll3,ll4,lm,ln,ln2,ln3,lo,lp,lp2,lq,lq2,lr,lr2,ls,ls2,lt,lu,lu2,lv,lw,lx,ly,lz]
...
function draw() {
	...
	let sgmtX = allLetters.length
	let sgmtY = allLetters.length
	
	//let sgmtX = 5
	//let sgmtY = 4
	
	translate(width/(sgmtX+1), height/(sgmtY+1)) //, height /2);
	scale(.5)
	//scale(1/(sgmtX-1)*1.25);
	translate(-width/(sgmtX+1), -height/(sgmtY+1)) //, -height / 2);
	...
	for(let i=0; i<sgmtX; i++){ 
		...
		*/
			...
			let letterLocs = []
			/*
			for (let lNum = 0; lNum < allLetters.length; lNum++) {
				letterLocs[lNum] = allLetters[lNum]
			}
			*/
			letterLocs[0] = lk
			letterLocs[1] = la
			...
			let useLetter = j //min(j+50,allLetters.length-1) //gridLoc(i*j,sgmtX*sgmtY)

				ltrPts = allLetters[useLetter].length
				...
				for (let lLen = 0; lLen < ltrPts; lLen++) {
					
					loc = allLetters[useLetter][lLen]
					//loc = letterLocs[lRan][lLen]
