...
let locRow, locCol, pos, loc, points, newPtx, newPty, totalPts, p, q, w, boxSize, lastDir, newDir, newLocLastDir, sqColor, offset, dotMixRan, seg
...
function setup() {
	...
	let gridSize = random([6,10,15]); //grid for the letter to be drawn on variable that can be changed. recommend 5-7 for letters
	//maybe make seg options relative to gridSize??
	if (gridSize == 6) { //make segment options relative to gridSize - gridSize 6 too basic in seg 10, gridSize 15 too hard to read in seg 30
		seg = random([20,25,30]);
	} else if (gridSize == 10) {
		seg = random([15,20,25]);
	} else {
		seg = random([10,15,20]);
	} //size of grid for letters to be written on across the page; min 30 for letter grid...looks better with more
	let totBox = gridSize*gridSize; //total number of squares in grid; don't change
	let ltrPts = floor(totBox * random([0.3,0.5,0.7])); //how many possible squares can be filled. *0.2 should be minimum; 0.3 - 0.6 seems like good range. max 0.8?
	let samDirWt = random([2,7,15]); //higher number = straighter lines; lower = squigglier; rec min 2 max 15(?)
	...
	let boxSize = (bgWidth / gridSize / seg);
	
	let dotMix = floor(random(4)) //dot vs line settings: 0 = all dot, 1 = mix dot-line, 2-3 = all line
	...
	for(let i=0; i<seg; i++){ 
		...
		for(let j=0; j<seg; j++){
			...
			for (let ll = 0; ll < letterLocs.length; ll++) {
				...
				dotMixRan = Math.random() //random setting for mixDot == 1, dot/line variation
				...
				} else if (dotMix == 1 && dotMixRan < 0.5) {
					translate(posx1, posy1); //makes dots/dashes instead of continuous lines
					scale(0.5); 
					translate(-posx1, -posy1);
				}
